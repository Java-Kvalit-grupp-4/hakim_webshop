    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          swal({
            title: "Något gick fel vid inläsning av kunder",
            icon: "warning",
          });
        }
      })
...
function showCustomers(customerArr) {
  ...
  customerArr.forEach((customer) => {
    ...
    axios
      .get(getCustomerOrder + customer.email, config)
      .then((response) => {
        if (response.status === 200) {
          let numberOfOrders;
          let totalPrice;
...
function saveCustomer() {
  ...
  customers.forEach((customer) => {
    if (customer.customerNumber == customerNumber) {
      let newZipCode = "";
      let newPhoneNumber = `${customer.phoneNumber.substring(
        0,
        3
      )} ${customer.phoneNumber.substring(
        3,
        6
      )} ${customer.phoneNumber.substring(
        6,
        8
      )} ${customer.phoneNumber.substring(8)}`;
      sessionStorage.setItem("choosedCustomer", JSON.stringify(customer));

      if (customer.zipCode != null) {
        newZipCode = `${customer.zipCode.substring(
          0,
          3
        )} ${customer.zipCode.substring(3)}`;
      }
      axios
        .get(getCustomerOrder + customer.email, config)
        .then((response) => {
        ...
        axios.get(getAllOrders, config).then((response) => {
          ...
          if (startDateC != null && endDateC != null) {
            filterOrders.forEach((order) => {
              ...
              date.setHours(10);
              if (
                date >= startDateC.setHours(0) &&
                date <= endDateC.setHours(23)
              ) {
