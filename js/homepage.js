...
function fetchCustomerInfo(customer, openPage) {
  axios.get(getCustomer + customer.email).then((response) => {
...
const removeHiddenProductsFromArray = (array) =>
  array.filter((product) => product.isAvailable == true);
    ...
    $("#products").append(`
        ...
        <div class="product-card">
            <div id="${element.sku}">
                    ...
                    <h3 id="${element.brand.name}" class="card-title">${
      element.title
    }</h3>
                    ...
                    <p id="${
                      element.volume
                    }" class="card-comp-price">Jfr-pris ${Number(
      element.comparablePrice
      ...
      maximumFractionDigits: 2,
                    ...
                    <div class="add-product-to-cart-container">
                        <button class="add-product-to-cart" style="margin-top: 5%" id=${
                          element.sku
                        }>Köp</button>
                    </div>
                    </div>
    ...
    axios
      ...
      .catch((err) => {
        if (err.response.status == 400) {
          swal(
            "E-post registrerad",
            "Det finns redan ett konto registrerat med denna e-post,\n vänligen logga in eller använd en annan e-post",
            "warning"
          );
        } else {
